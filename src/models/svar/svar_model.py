...
class SVARModel:
    ...
    def historical_decomposition(self, start_date: str = None, 
        ...
        # Check if we have valid structural shocks
        if structural_shocks.size == 0:
            raise ValueError("No structural shocks available for decomposition")
    ...
    def _recover_structural_shocks(self) -> np.ndarray:
        ...
        if reduced_form_residuals.size == 0:
            raise ValueError("No residuals available from VAR model")
        ...
        if structural_shocks.size == 0:
            raise ValueError("Structural shocks computation resulted in empty array")
    ...
    def _compute_shock_contributions(self, structural_shocks: np.ndarray) -> np.ndarray:
        ...
        if n_obs == 0 or n_vars == 0:
            raise ValueError("Empty structural shocks array")
        
        # Simplified approach: direct impact of shocks
        # Each shock contributes through the structural matrix
        contributions = np.zeros((n_obs, n_vars, n_vars))
        
        for t in range(n_obs):
            for shock_idx in range(n_vars):
                # Direct impact of shock on all variables
                contributions[t, :, shock_idx] = (
                    self.structural_matrix[:, shock_idx] * structural_shocks[t, shock_idx]
                )
    ...
    def monetary_transmission_analysis(self) -> Dict:
        ...
        # Find a suitable monetary policy variable
        policy_vars = ['policy_rate', 'interest_rate', 'money_growth', 'inflation']
        shock_var = None
        ...
        if shock_var is None:
            # Use the first variable as proxy
            shock_var = self.variables[0]
        
        # Compute monetary policy IRFs
        mp_irf = self.impulse_response(shock_variable=shock_var, periods=20)
...
if __name__ == "__main__":
    ...
    data = pd.DataFrame({
        ...
        'unemployment': np.random.normal(0.045, 0.01, n_obs),
        'exports_growth': np.random.normal(0.08, 0.03, n_obs)
    }, index=dates)
    ...
    print(f"Variables used: {svar_model.variables}")
    ...
    # Compute impulse responses using the first variable as shock
    shock_var = svar_model.variables[0] if svar_model.variables else 'gdp_growth'
    irf = svar_model.impulse_response(shock_variable=shock_var, periods=16)
    print(f"\nIRF computed for {shock_var}: {irf.shape}")
    
    # Compute forecast error variance decomposition
    try:
        fevd = svar_model.forecast_error_variance_decomposition(periods=12)
        print(f"\nFEVD computed: {fevd.shape}")
    except Exception as e:
        print(f"\nFEVD computation failed: {e}")
    
    # Historical decomposition
    try:
        hist_decomp = svar_model.historical_decomposition()
        print(f"\nHistorical decomposition computed: {hist_decomp.shape}")
    except Exception as e:
        print(f"\nHistorical decomposition failed: {e}")
    ...
    try:
        ...
        print(f"\nMonetary transmission analysis failed: {e}")
    ...
    try:
        ...
        # Combine all results into a comprehensive DataFrame
        results_data = {
            'metric': [],
            'value': [],
            'description': []
        ...
        # Add model summary metrics
        results_data['metric'].extend(['log_likelihood', 'aic', 'bic', 'sample_size', 'n_variables'])
        results_data['value'].extend([svar_model.svar_results.log_likelihood, svar_model.svar_results.aic, svar_model.svar_results.bic, 
                                    summary['sample_size'], len(summary['variables'])])
        results_data['description'].extend(['Log-likelihood', 'Akaike Information Criterion', 
                                          'Bayesian Information Criterion', 'Sample Size', 'Number of Variables'])
        
        # Add IRF results summary
        if 'irf' in locals():
            irf_max = irf.max().max()
            irf_min = irf.min().min()
            results_data['metric'].extend(['irf_max_response', 'irf_min_response'])
            results_data['value'].extend([irf_max, irf_min])
            results_data['description'].extend(['Maximum IRF Response', 'Minimum IRF Response'])
        
        # Add FEVD results summary
        if 'fevd' in locals():
            fevd_mean = fevd.mean().mean()
            results_data['metric'].extend(['fevd_average'])
            results_data['value'].extend([fevd_mean])
            results_data['description'].extend(['Average FEVD Contribution'])
        
        # Create and save results DataFrame
        results_df = pd.DataFrame(results_data)
        csv_path = results_dir / "svar_results.csv"
        results_df.to_csv(csv_path, index=False)
        print(f"\nResults exported to: {csv_path}")
        
        # Also save detailed IRF results if available
        if 'irf' in locals():
            irf_path = results_dir / "svar_irf_results.csv"
            irf.to_csv(irf_path)
            print(f"IRF results exported to: {irf_path}")
        
        # Save FEVD results if available
        if 'fevd' in locals():
            fevd_path = results_dir / "svar_fevd_results.csv"
            fevd.to_csv(fevd_path)
            print(f"FEVD results exported to: {fevd_path}")
            
    except Exception as e:
        print(f"\nFailed to export results to CSV: {e}")